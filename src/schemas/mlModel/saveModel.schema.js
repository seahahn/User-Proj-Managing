...
import { deleteModelInS3, S3_OBJECT_URL } from "../utils/s3.js";
...
const saveModel = {
  ...
  handler: function (req, rep) {
    ...
    const modelUrl = S3_OBJECT_URL + userIdx + "/" + modelName;
    ...
    this.pg.query("SELECT * FROM ml_model WHERE user_idx=$1 AND model_name=$2", [userIdx, modelName], (err, result) => {
      err
        ...
        ? // 기존에 존재하지 않는 모델명이면 새로운 모델 정보 추가
          this.pg.query("INSERT INTO ml_model (user_idx, model_name, model_url) VALUES ($1, $2, $3)", [userIdx, modelName, modelUrl], (err, result) => {
            if (err) {
